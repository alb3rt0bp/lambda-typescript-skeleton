...
const _insertOrUpdateDB = (user: User, logData: LogData): Promise<User> => {
...
const _listUsers = (logData: LogData): Promise<User[]> =>{
    return new Promise((resolve, reject) => {
        const dynamodb = new AWS.DynamoDB({apiVersion: '2012-08-10', region: config.aws.region})
...
const _userDetail = (id:string, logData: LogData): Promise<User> =>{
    return new Promise((resolve, reject) => {
        const dynamodb = new AWS.DynamoDB({apiVersion: '2012-08-10', region: config.aws.region})
...
const _deleteUser = (id:string, logData: LogData): Promise<void> =>{
    return new Promise((resolve, reject) => {
        const dynamodb = new AWS.DynamoDB({apiVersion: '2012-08-10', region: config.aws.region})
...
export async function createUser(event: AWSLambda.APIGatewayEvent, logData: LogData): Promise<AWSLambda.APIGatewayProxyResult> {
...
export async function listUsers(event: AWSLambda.APIGatewayEvent, logData: LogData): Promise<AWSLambda.APIGatewayProxyResult> {
    const startTime = process.hrtime()
...
export async function userDetail(event: AWSLambda.APIGatewayEvent, logData: LogData): Promise<AWSLambda.APIGatewayProxyResult> {
    const startTime = process.hrtime()
...
export async function deleteUser(event: AWSLambda.APIGatewayEvent, logData: LogData): Promise<AWSLambda.APIGatewayProxyResult> {
    const startTime = process.hrtime()
    // @ts-ignore. Ignored because event.pathParameters are already sanitized with schema in app/schemas/userDetail.json
    logger.info(logData, `app.controller.deleteUser: Request received for User ${event.pathParameters.id}`)
    // @ts-ignore. Ignored because event.pathParameters are already sanitized with schema in app/schemas/userDetail.json
    return _deleteUser(event.pathParameters.id, logData)
        .then(() => {
            const response: AWSLambda.APIGatewayProxyResult = {
                statusCode: 204,
                body: ''
...
export async function updateUser(event: AWSLambda.APIGatewayEvent, logData: LogData): Promise<AWSLambda.APIGatewayProxyResult> {
    const startTime = process.hrtime()
    // @ts-ignore. Ignored because event.pathParameters are already sanitized with schema in app/schemas/userDetail.json
    logger.info(logData, `app.controller.deleteUser: Request received for User ${event.pathParameters.id}`)
